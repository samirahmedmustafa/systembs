...
public interface CitizensRepo extends JpaRepository<Citizens, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT"
			...
			+ "	count(distinct citizens_wives.wife_id) wivesCount"
			+ " FROM citizens"
			...
			+ "	left outer join citizens_wives on citizens.id=citizens_wives.husband_id"
			...
			+ "	left outer join school on citizens.school_id=school.id" + " group by"
			+ "	citizens.id,gender.name,location.number,nationality.name,qualification.name,school.name")
	List<Object[]> findCustomCitizens();

	@Query("SELECT p FROM Citizens p WHERE lower(p.gender.name) = lower(:gender)")
	List<Citizens> findByGender(@Param("gender") String gender);
	@Query("SELECT p FROM Citizens p WHERE lower(p.gender.name) = lower('female')")
	List<Citizens> findByFemale();
